...
# ── Résolution robuste du chemin (local + Streamlit Cloud) ───────
_THIS_DIR = Path(__file__).resolve().parent
os.chdir(_THIS_DIR)                          # Streamlit Cloud fix
...
if not articles:
    st.error(
        "❌ Aucune donnée récupérée. Vérifiez vos clés API dans le fichier `.env` "
        "(NEWSAPI_KEY ou GNEWS_KEY requis)."
    )
    st.stop()
